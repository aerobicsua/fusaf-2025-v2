...
const demoCompetitions: Competition[] = [
  {
    ...
    rules: 'Змагання проводяться згідно з правилами FIG та ФУСАФ. Вік учасників визначається на 31 грудня 2025 року. Обов\'язкова наявність медичної довідки та страхування.',
    contact_person: {
    ...
    prizes: [
      '🥇 1 місце - Кубок, медаль, диплом та грошова премія 5000 грн',
      '🥈 2 місце - Медаль, диплом та грошова премія 3000 грн',
      '🥉 3 місце - Медаль, диплом та грошова премія 2000 грн',
      '🏆 Спеціальні номінації для всіх учасників'
    ],
    schedule: {
  ...
  },
  {
    ...
    rules: 'Регіональний чемпіонат для спортсменів Львівської області. Можуть брати участь клуби з інших областей як гості.',
    contact_person: {
    ...
    prizes: [
      '🥇 1 місце - Кубок та медаль',
      '🥈 2 місце - Медаль та диплом',
      '🥉 3 місце - Медаль та диплом'
    ],
    schedule: {
      registration: '08:00 - 09:00',
      warmup: '09:00 - 09:30',
      competition: '09:30 - 15:00',
      awards: '15:00 - 16:00'
    }
  },
  {
    ...
    rules: 'Відкрита першість з можливістю участі для всіх клубів України. Змагання проводяться за новими правилами ФУСАФ 2025.',
    contact_person: {
    ...
    prizes: [
      '🏆 Кубки для призерів у всіх категоріях',
      '🎖️ Медалі для всіх учасників',
      '📜 Дипломи та сертифікати'
    ],
    schedule: {
      registration: '09:30 - 10:30',
      warmup: '10:30 - 11:00',
      competition: '11:00 - 17:00',
      awards: '17:00 - 18:00'
    }
  }
];
...
export default function CompetitionsPage() {
  ...
  const canUserRegisterTeams = (competition: Competition) => {
    ...
    return session && isRegistrationAvailable(competition) && canRegisterTeams(session.user?.roles);
  };

  const canUserRegisterIndividual = (competition: Competition) => {
    // Іменну реєстрацію можуть подавати всі авторизовані користувачі
    return session && isRegistrationAvailable(competition) && canRegisterIndividual(session.user?.roles);
  };

  const canUserCreateCompetition = () => {
    // Тільки власники клубів та адміністратори можуть створювати змагання
    const hasPermission = session && session.user?.roles?.some(role => ['admin', 'club_owner'].includes(role));
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {userCanRegisterIndividual &&
                        session?.user?.roles?.some(role => ['admin', 'club_owner', 'coach_judge'].includes(role)) && (
                        <div className="mt-2">
                          <Dialog>
                      ...
                      {session?.user?.roles?.length === 1 &&
                        session?.user?.roles?.includes('athlete') &&
                        !canRegisterTeams(session?.user?.roles) && (
                      ...
                      {session?.user?.roles?.some(role => ['admin', 'club_owner'].includes(role)) && (
