...
export function RequestRoleUpgrade() {
  ...
  if (!session?.user ||
      (!session.user.roles?.includes('athlete') && !session.user.roles?.includes('admin')) ||
      hasSubmitted) {
    return null;
  }
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700">
                  Запит на роль "{ROLE_NAMES[session.user.roleRequest?.requestedRole as keyof typeof ROLE_NAMES] || session.user.roleRequest?.requestedRole || 'Невідома роль'}" очікує розгляду
                </p>
                <p className="text-xs text-yellow-600 mt-1">
                  Подано: {session.user.roleRequest?.requestDate ? new Date(session.user.roleRequest.requestDate).toLocaleDateString('uk-UA') : 'Невідомо'}
                </p>
              </div>
            </div>
          </div>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogTrigger asChild>
            <Button
              ...
              disabled={session?.user?.roleRequest?.status === 'pending'}
            >
              <ArrowUp className="h-4 w-4 mr-2" />
              {session?.user?.roleRequest?.status === 'pending'
                ? 'Запит вже подано'
